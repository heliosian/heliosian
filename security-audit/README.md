...
- Cookie attributes and scope: `Secure`, `HttpOnly`, `SameSite`, lifetime, and the domain. The session and the spoof cookie are tier-wide, so every app's origin carries them; check what production's cookies are sent to, given that `*.local.heliosian.com` sits under the production domain and resolves to the reader's own machine.
