...
Severity: high
---
`blob.Thumbnail` (`internal/blob/blob.go`) calls `image.Decode` on the bytes as they arrive; `Store.Put` and `PutNamed` reach it through `writeWithThumbnail` (`blob.go`). The callers check only the first bytes' sniffed type and a byte cap (8 MB, 30 MB in Who?): `team.uploadImage` (`internal/team/team.go`), Celebrate's `uploadImage` (`internal/celebrate/handlers.go`), `imagesearch.ServeImport` (`internal/imagesearch/imagesearch.go`) on the portal, Celebrate, When and Heliosian, and Who?'s `upload` and `cropPhoto` (`internal/who/upload.go`). None of the portal's, Celebrate's or When's routes asks for a role. `internal/sharecard/card.go` decodes a stored picture again on the public `/open/share/` routes.
