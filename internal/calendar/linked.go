...
// HCA-Team event - as the calendar lists it: what and when, its page on that
// site, and what a reader can do there now.
type Linked struct {
	Source       string
	ID           string
	Title        string
...
// sheet row names: one per app, and Misc for an event the sheet gave an
// audience but no category. Every view carries them beside the sheet's own
// so the rail can switch each off.
var builtinTags = []Tag{
...
func linkedEvent(l Linked) *Event {
	...
	e := &Event{
		...
		Link: l.Path, Availability: l.Availability, start: start, end: end,
	}
	if e.End == "" {
		e.End = e.Start
	}
...
// its availability, and the HCA tag - on a copy, so the model's own event
// stays as loaded.
func folded(school, hca *Event) *Event {
	c := *school
	c.Tags = append(append([]string{}, school.Tags...), TagHCA)
	c.Link, c.Availability = hca.Link, hca.Availability
	return &c
}
