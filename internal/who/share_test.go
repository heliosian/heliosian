...
func TestSharePreview(t *testing.T) {
	...
	if tags := head(httptest.NewRequest("GET", "https://who.heliosian.com/", nil)); !strings.Contains(tags, `og:title" content="Renamed"`) || !strings.Contains(tags, `content="A new line. `) {
	...
	for _, path := range []string{"/", "/some/page"} {
		tags := head(httptest.NewRequest("GET", "https://who.heliosian.com"+path, nil))
		for _, want := range []string{`og:site_name" content="Helios Who?"`, `og:title" content="Helios Who?"`, `og:url" content="https://who.heliosian.com/"`, `og:image" content="https://who.heliosian.com/open/share/about.png"`} {
