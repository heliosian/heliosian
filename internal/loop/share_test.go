...
func TestSharePreview(t *testing.T) {
	...
	if tags := head(httptest.NewRequest("GET", "https://loop.heliosian.com/", nil)); !strings.Contains(tags, `og:title" content="Renamed"`) || !strings.Contains(tags, `content="A new line. `) {
	...
	for _, path := range []string{"/", "/some/page"} {
		tags := head(httptest.NewRequest("GET", "https://loop.heliosian.com"+path, nil))
		for _, want := range []string{`og:site_name" content="Helios Loop"`, `og:title" content="Helios Loop"`, `og:url" content="https://loop.heliosian.com/"`, `og:image" content="https://loop.heliosian.com/open/share/about.png"`} {
