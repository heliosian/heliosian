...
func TestSharePreview(t *testing.T) {
	...
	for _, path := range []string{"/", "/some/page"} {
		...
		for _, want := range []string{`og:site_name" content="Helios Loop"`, `og:title" content="One address reaches the whole group`, `og:url" content="https://loop.heliosian.com/"`, `og:image" content="https://loop.heliosian.com/open/share/about.png"`} {
