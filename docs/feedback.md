...
The handler validates, commits the report as a new row of the Reports tab, made as the reporter, and answers `202 Accepted`; the commit takes the report into memory at once and writes the row behind the request (`docs/storage.md`), and the mail to the super admins goes out after it, neither of which the reporter waits on. A commit that is refused answers 503 and logs the whole report at error level, so nothing is lost. A person may send `perWindow` reports in each `window` (both in `internal/feedback/feedback.go`, counted by `internal/ratelimit`); one more gets a 429 asking them to wait.
