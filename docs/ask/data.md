...
Helios Ask has no sheet. It reads every other app's model through `ask.Sources`, which `internal/app` builds from the caches the way the apps' own adapters are built (`internal/app/ask.go`), and it talks to Claude through the Anthropic API. This file carries what reading `internal/ask` cannot tell you.
...
Every event, party and volunteer thing carries `past` and `whenAgainstToday` ("past (8 days ago)", "today", "tomorrow", "in 12 days"), reckoned here so the model never does date arithmetic, and each of those tools says what today is. The words the page shows for a lookup are named once per turn however many times the tool ran.
