...
A handler states a change as operations on rows - `store.Insert`, `store.Set` (every matching row, or a new one when none matches), `store.Update` (every matching row, and nothing when none does), `store.Delete` - and hands them to `Commit(ctx, actor, ops...)`. A commit has two halves that never wait on each other. Under the one commit lock every store shares, on `store.Queue`, it:
...
4. abandons a refresh under way (below), swaps the tables and model in, and queues the IO writes and the Change Log rows on the one write queue every store shares (`store.Queue`);
5. releases the lock and returns.

The memory half takes milliseconds and waits only on another commit's memory half, in any store; the IO half runs on the queue in commit order, and no request waits on it. The queue's one goroutine is the only thing that writes a sheet. Writes to one tab that follow each other go as one request: inserts as one Insert, and sets and updates that each name a row by a single column, and leave that column alone, as one SetMany - so an import that re-classifies every event, or a send that marks a whole guest list, is a handful of requests rather than one per row. An IO write the sheet refuses, the Change Log's included, ends the process there (`logging.Fatal`): memory is ahead of the sheet, and any write queued behind may build on the refused one, so none may run; the sheet keeps an unbroken prefix of what was committed, and the next start reads it. A mail hook - Loop's received mail, Helios When's calendar replies and bounces, the mail Helios Ask files among its documents - is the one caller on a request that waits for its IO: it commits with `CommitAndWait`, which takes the same place in the queue as `Commit` and then waits for that write to finish, never jumping ahead of what was queued before it. So the hook answers the mail provider only once what it took is in the sheet; a refused write ends the process before it answers, and the provider retries. The calendar, directory and documents imports, and Loop's filing of a sent post, wait the same way.
...
A store reads its sheet at startup. After that every sheet is read together, in one refresh: one ticker, every five minutes (`Queue.Tick`), and once more thirty seconds after a start (`deployOverlap` in `internal/app`) - during a deploy the old revision keeps serving and writing while the new one reads, and that second read picks up what it wrote. The five-minute refresh is for what changes in the sheets by hand. A refresh is a task on the write queue: it reads and builds every store's model, and the media store's list of what is named, then swaps them all in at once, or none of them if any read or build fails. A commit that lands while it reads cancels it (the refresh's context), since its write waits behind the refresh on the queue and the sheet just read cannot hold it; the next tick reads again. Work accepted but not yet committed - a mail the documents' hook took, a group's post being filed - holds the queue (`Hold` and `Release` in `internal/store/queue.go`), so the shutdown drain waits for it.

A model is built from the tables alone, but for one app: Helios Ask's documents are objects in the bucket that the `Documents` tab only names. Its build reads each object a row names, keeping the documents it has read by object name, so a refresh fetches only the rows it has not seen; filing a document puts it in that memo before the commit, so a commit never waits on the bucket. Each load that swaps in - startup and every refresh - cuts the memo back to the documents its model holds, so a document whose row is gone leaves memory at the next refresh. A document filed while a refresh swaps in, ahead of its commit, can be cut with it; its commit then reads it from the bucket once.
