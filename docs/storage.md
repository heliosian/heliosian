...
A model is built from the tables alone, but for one app: Helios Ask's documents are objects in the bucket that the `Documents` tab only names. Its build reads each object a row names, keeping every document it has read by object name, so a refresh fetches only the rows it has not seen; filing a document puts it in that memo before the commit, so a commit never waits on the bucket.
