...
- `Settings` - Key, Value: `Parties Intro`, `Ticket Note`, and `Hosting Open` (Yes or No; blank means Yes - whether anyone may post a party), all optional; saving from Admin Tools appends a key the tab lacks. Also the seven theme keys every app shares (`Sidebar Color`, `Sidebar Color 2`, `Sidebar Text Color`, `Page Color`, `Page Color 2`, `Logo`, `Sidebar Image` - `internal/theme`; see `docs/home/data.md`), written by the Appearance panel of Admin Tools, appended when the tab lacks them.
- `Admins` - Email.
