...
- `Settings` - Key, Value: `Parties Intro`, `Ticket Note`, and `Hosting Open` (Yes or No; blank means Yes - whether anyone may post a party), all optional; saving from Admin Tools appends a key the tab lacks. Rows under the old theme keys (`Sidebar Color` and the like, from the Appearance panel that existed on 2026-09-16) are passed over if the tab still holds them.
