...
- `Visibility` — App, Visibility, Emails, Tagline. One row per community app (`who`, `team`, `celebrate`, `birthday`): whether everyone sees it or only the people in its Emails cell - in the app switch of every app's toolbar, and on the front page's apps section - and the line under its name there. The server adds the row for an app that has none.
