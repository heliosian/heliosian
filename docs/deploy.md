...
A push to `main` on `github.com/heliosian/heliosian` is meant to deploy production, through two Cloud Build triggers in `us-west1`, attached to the repository through a GitHub connection, each carrying its build config inline — nothing in the repo describes a build. What each needs to do:

- `build-main` — `docker build` of the repo's Dockerfile, tagged with the commit SHA and `latest`; push the SHA tag to Artifact Registry (`us-west1-docker.pkg.dev/heliosian/heliosian/heliosian`); `gcloud run deploy heliosian --image …:<sha>` — image only, so every other service setting persists from the last full deploy.
- `build-periodicsync` — the same three steps for the periodic sync job (below): `docker build` of `Dockerfile.periodicsync`, a push to `us-west1-docker.pkg.dev/heliosian/heliosian/periodicsync`, and `gcloud run jobs deploy periodicsync --image …:<sha>`, which changes only the job's image. The job has to exist first, from a `cmd/deploy` run: a job created with nothing but an image would run as the default compute account, which the build account may not act as, so the step fails until the job is there with its identity.

Each trigger has to name its build service account (the project's default compute service account) explicitly; trigger creation in this project refuses to infer one. The triggers themselves are the only record of what a build does — this page says what they need to do, never what they do — so read them, and the recent builds with each one's outcome beside them, rather than this page:
...
- `heliosian-mailgun-key` — the Mailgun API key (as `MAILGUN_KEY`) every app's mail goes out through, mirrored locally as `creds/mailgun.key`. What the apps need of the Mailgun account (US region) is written here as needs, never as what the account holds - the dashboard and the API, read below, are the only record of that. `reply.heliosian.com` is the domain every app sends from - the volunteer portal as `HCA-Team <team@reply.heliosian.com>` (override with `MAIL_FROM`), Helios Celebrate as `Helios Celebrate <celebrate@reply.heliosian.com>` (`CELEBRATE_MAIL_FROM`), Staff Birthdays as `Helios Staff Birthdays <birthday@reply.heliosian.com>` (`BIRTHDAY_MAIL_FROM`; `BIRTHDAY_BASE_URL`, default `https://birthday.heliosian.com`, is the address its reminders link to), Helios Who? as `Helios Who? <who@reply.heliosian.com>` (`WHO_MAIL_FROM`) and Helios Calendar as `Helios When <when@reply.heliosian.com>` (`CALENDAR_MAIL_FROM`), which its invites also name as their organizer and reply address (`CALENDAR_REPLY_TO`, the same address, since a calendar app may answer the sender rather than the organizer). So it needs to be verified for sending - its DKIM record and its SPF `include` at the registrar, where the wildcard CNAME otherwise answers for the name - and set up to receive, its MX records pointing at Mailgun, so that the Accept or Decline a person taps in their calendar app lands on Mailgun whichever address it answers; and it needs one route matching every recipient under the domain that stores the message with a notification to `https://when.heliosian.com/api/calendar/replies` (`docs/calendar/calendar.md`, Answering), which reads the calendar's replies and drops everything else. `heliosian.com` is a domain no app may send from: its MX belongs to Google Workspace, where the school reads mail to its addresses, so an address there on an app's mail invites replies into an inbox nobody watches. `loop.heliosian.com` is Helios Loop's, and needs the same: sending verified, MX records at Mailgun (which is what takes the domain's mail away from Google Workspace, where it is otherwise a secondary domain), a route matching every recipient under it that stores the message with a notification to `https://loop.heliosian.com/api/loop/mail`, and the domain's webhooks for permanent failure, temporary failure and complaint pointing at `https://loop.heliosian.com/api/loop/events` (`docs/groups/data.md`, Mail). What the account holds, read with the local key:
  ...
  Mail that has gone out, with each bounce and complaint, is under the domain's events - `https://api.mailgun.net/v3/reply.heliosian.com/events` with the same key, or Sending › Logs in the dashboard. SMTP is the fallback when there is no Mailgun key: `SMTP_HOST`, `SMTP_PORT` (587), `SMTP_USER` and a `heliosian-smtp-pass` secret as `SMTP_PASS`. With neither the apps send nothing, and the portal says so in Admin Tools.
- `heliosian-mailgun-webhook-key` — Mailgun's HTTP webhook signing key (as `MAILGUN_WEBHOOK_KEY`), from the account's Webhooks page, which signs every route's notifications and every domain's event webhooks. Mirrored locally as `creds/mailgun-webhook.key`. Without it, or without the API key, the calendar's reply route and Loop's two routes answer 404: replies go unread and mail to the groups goes nowhere.
...
- `heliosian-anthropic-key` — the Anthropic API key (as `ANTHROPIC_API_KEY`) the calendar import job classifies and reads with, and the service uses for Staff Birthdays' one-sentence charity descriptions (`docs/birthday/birthday.md`); mirrored locally as `creds/anthropic.key`. The key never reaches a browser: the description is asked for by the server, on a button press, by someone signed in
...
Helios Loop keeps every message its groups receive in a second bucket, `gs://heliosian-mail`, which needs the same settings as the media bucket (the same `describe` and `get-iam-policy`, with its name, read them), whole as it came, under `loop/<group>/` (`docs/groups/data.md`, Mail). It is its own bucket because nothing in the media bucket's serving routes may ever reach a message: the mail bucket has no route at all, and the `Messages` tab of the Groups sheet is its index. Make it as the media bucket was made, with `roles/storage.objectAdmin` on it to `directory@`.
