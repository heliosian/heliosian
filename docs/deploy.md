...
- `heliosian-mailgun-key` — the Mailgun API key (as `MAILGUN_KEY`) Helios Loop fetches its groups' posts back with and forwards them through, from `loop.heliosian.com`, a domain in Mailgun (US region) verified for sending: its DKIM record and its SPF `include` at the registrar, where the wildcard CNAME otherwise answers for the name, and its two MX records pointing at Mailgun, which is what takes the domain's mail away from Google Workspace, where `loop.heliosian.com` was a secondary domain, and from Resend's receiving. Mirrored locally as `creds/mailgun.key`. One route on the domain matches every recipient under it and stores the message with a notification to `https://loop.heliosian.com/api/loop/mail`; the domain's webhooks for permanent failure, temporary failure and complaint point at `https://loop.heliosian.com/api/loop/events` (`docs/groups/data.md`, Mail).
- `heliosian-mailgun-webhook-key` — Mailgun's HTTP webhook signing key (as `MAILGUN_WEBHOOK_KEY`), from the account's Webhooks page, which signs both the route's notifications and the domain's event webhooks. Mirrored locally as `creds/mailgun-webhook.key`. Without it, or without the API key, Loop's two routes answer 404 and mail to the groups goes nowhere. Resend still carries the calendar's replies at `reply.heliosian.com`, and its webhook, being account-wide, is unaffected by Loop's move.
