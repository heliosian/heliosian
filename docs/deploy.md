...
- `heliosian-mailgun-key` — the Mailgun API key (as `MAILGUN_KEY`) every app's mail goes out through, mirrored locally as `creds/mailgun.key`. Three domains are in Mailgun (US region), each verified for sending with its DKIM record and its SPF `include` at the registrar, where the wildcard CNAME otherwise answers for the name: `heliosian.com`, which the volunteer portal sends from as `HCA-Team <team@heliosian.com>` (override with `MAIL_FROM`), Helios Celebrate as `Helios Celebrate <celebrate@heliosian.com>` (`CELEBRATE_MAIL_FROM`), Staff Birthdays as `Helios Staff Birthdays <birthday@heliosian.com>` (`BIRTHDAY_MAIL_FROM`; `BIRTHDAY_BASE_URL`, default `https://birthday.heliosian.com`, is the address its reminders link to), Helios Who? as `Helios Who? <who@heliosian.com>` (`WHO_MAIL_FROM`) and Helios Calendar as `Helios Calendar <when@heliosian.com>` (`CALENDAR_MAIL_FROM`) - its MX stays with Google Workspace, which is where the school reads mail to those addresses; `reply.heliosian.com`, whose MX records point at Mailgun, which the calendar's invites name as their organizer and reply address, `Helios Calendar <rsvp@reply.heliosian.com>` (`CALENDAR_REPLY_TO`), so the Accept or Decline a person taps in their calendar app lands there, and a route matching every recipient under it stores the message with a notification to `https://when.heliosian.com/api/calendar/replies` (`docs/calendar/calendar.md`, Answering); and `loop.heliosian.com`, whose MX records point at Mailgun too, which is what took the domain's mail away from Google Workspace, where it was a secondary domain, for Helios Loop's groups, with a route matching every recipient under it storing the message with a notification to `https://loop.heliosian.com/api/loop/mail`, and the domain's webhooks for permanent failure, temporary failure and complaint pointing at `https://loop.heliosian.com/api/loop/events` (`docs/groups/data.md`, Mail). Every send, bounce and complaint shows in Mailgun's logs. SMTP is the fallback when there is no Mailgun key: `SMTP_HOST`, `SMTP_PORT` (587), `SMTP_USER` and a `heliosian-smtp-pass` secret as `SMTP_PASS`. With neither the apps send nothing, and the portal says so in Admin Tools.
