...
Every app, Admin Tools page and sign-in page takes its colours from one stylesheet, `web/public/common/palette.css`, linked first in each page's head: the brand teal, the accent teal, the page and card grounds, the lines, the ink, the muted text, the input ground, the one alert red, and the logo swatch's greens and golds. It sits in the public tree so the sign-in pages, served before sign-in, reach it too. An app's own `:root` holds only what is its alone, such as Helios When's day-type colours and its month's day washes. Dark mode (`web/common/dark.css`) and Quan mode (`web/common/quan.css`) redefine the same names on the page's root, so a surface drawn from the palette - the page, a card (`--card`), a form field (`--field`), the top bar (`--toolbar`), the yellow notices (`--notice-*`), the badges (`--badge`) - follows the mode on its own; a surface must never be painted white or pale by literal. Where dark mode wants a colour for a role whose day shade differs from place to place - a pale hover wash, a tinted band, a pale version of coloured words - the stylesheet reads a variable that only `dark.css` defines, with the day shade as its fallback: `var(--wash, …)` is a pale wash that turns to the input ground, `var(--card-wash, …)` one that turns to the card, `var(--hollow, …)` one that goes clear, `var(--brand-ink, var(--brand))` brand-coloured words that turn to plain ink, `var(--yes-text, …)` and its siblings the RSVP greens, ambers and reds, and so on. By day and in Quan those variables are unset, so the fallback holds. `dark.css` is therefore the palette's dark values and those dark-only variables, plus a handful of rules that still name a selector: the form controls' default ground (inputs the apps leave unstyled), the placeholder colour, and three dark-only touches with nothing to match by day (Staff Birthdays' picker chips gone solid, the glass buttons on Celebrate's host band, the dimmed room line on When's day card). Quan sets its own values for the notices and the bar; everything else it inherits from the palette. White is kept on purpose where it is a white thing on a coloured or photo ground rather than a page surface: switch knobs, crop handles, the tear-off date stamps and date leaves, the day picked in a rail's month, the date card and linked card over an event's banner, and Spoof Mode's pill on the pink bar. Each colour has one name, the role's where there is one: `--brand` for the teal as words and marks, `--brand-deep` for the deepest teal (the ink on a lime button, a tooltip's ground), `--brand-dark` for the darker shade a brand fill takes on hover, `--amber` for the pending and today gold, `--green-2` and `--green-3` for the buttons and their hover. Dark mode pales `--brand` so it reads as words on the dark page, so a teal ground that carries white words uses `--brand-fill` (defined in `palette.css`), which stays deep in dark mode and turns pink with the rest in Quan. Stylesheets write colours as these variables rather than as literals; the categorical `--cat-*` colours are used by name where a category takes one. A `theme-color` meta tag and a manifest's colours cannot read a stylesheet, so they carry the brand teal as a literal.
