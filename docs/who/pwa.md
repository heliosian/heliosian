...
- `manifest.webmanifest` at `/manifest.webmanifest`, a file in `web/who/`. Every file served from `web/` is public, which matters here because browsers fetch manifests without credentials. `scope` and `start_url` are `/`; name, `display: standalone`, theme and background color `#014E54`; icons 192 and 512 as `any` plus a 512 maskable (the maskable art keeps the lockup inside the safe zone on a full-bleed teal square). The server registers the `application/manifest+json` MIME type.
...
- Splash screens for iOS: both templates carry the original's full `apple-touch-startup-image` battery — 32 pre-rendered PNGs (the logo lockup on teal) covering every iPhone/iPad class in both orientations, served from `/brand/splash/` out of `web/who/brand/splash/`. `cmd/splash` regenerates them by extracting the link matrix and images from the captured original page source.
