...
- `manifest.webmanifest` at `/manifest.webmanifest`, a file in `web/public/who/`, the tree served without sign-in, because browsers fetch manifests without credentials. `scope` and `start_url` are `/`; name, `display: standalone`, theme and background color the brand teal (`docs/who/design.md`, Palette); icons 192 and 512 as `any` plus a 512 maskable (the maskable art keeps the lockup inside the safe zone on a full-bleed teal square). The server registers the `application/manifest+json` MIME type.
