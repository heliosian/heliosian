...
- Self-service: viewing your own record, your household's (an adult edits the children and the other adults; a student edits only themselves, not their siblings or parents), or your family page shows inline edit affordances — photo upload, pronunciation recording or upload, About Me text, preferred name, phone, address — plus opt-out for yourself or your kids. A media upload adds an object and a row naming it, never replacing what was there; sheet-backed edits are commits on the directory's store to the Overrides tab (person fields) or the Families tab (family fields), which log each changed cell's previous value in the Change Log (see `docs/who/data.md`, Every write is a commit). A facts edit or photo upload also stamps that item's updated date.
