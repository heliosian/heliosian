...
- `Activities` — Event ID, Year, Title, Parent, Category, Status, Description, Image, Timing, Start, End, Location, Spots, Co-Leader Needed, Volunteers Hidden, Direct Sign-Up, Pretty ID, Allow Adding, Flyer Image, Highlight Headline, Highlight Body, Highlight Icon, Added By, Added. `Image` is the wide banner across the page and the card; `Flyer Image` is the event's poster, shown whole in the page's rail and, when present, on the share card instead of the banner. Both are stored image names, uploaded, found in the image libraries, or cropped from the page. `Order` is optional: a number that places a thing among its siblings (lower first, blank after every numbered one, then row order), written by the Move up/down arrows and drag-and-drop in edit mode - a tab without the column lists children in row order and refuses to reorder. `Volunteers Complete` is optional too: Yes when the organizers say the thing is staffed, whatever Spots and the count say - it wears the green Volunteers Complete check (which a thing whose spots are all taken wears as well) and takes no more sign-ups; set by a checkbox in the editor's Sign-ups tab and beside Set Max while editing the page, and a copy for next year starts with it off. A tab without the column can only fill up by count. `tools/createtabs` adds both optional columns to an existing tab. The three `Highlight` columns are the page's callout - a headline, a few lines (with `**bold**` allowed) and an emoji - blank when there is none; a row with neither headline nor body has no highlight. Parent is an Event ID. Category is a Category ID: a page heading for a root, one of the root event's own for anything under it (or blank).
...
- `Change Log` — Timestamp, Actor, Action, Kind, Year, Activity, Title, Email, Details, Real Actor; appended on every change, never read back. Real Actor is who was signed in when Actor is who the change was made as, the two differing under Spoof Mode (`docs/toolbar.md`).
