...
- **A newcomer** is asked, the first time the app opens for them, whether they want to be on the birthday team; Join the Team puts them on the Team tab as a volunteer and on the app's list in Heliosian's settings (`home.Grant`), so the app shows on their home; Not now is remembered on that browser, and someone already on the team is never asked.
...
- **My Jobs** (`/jobs`, and the front page once everyone has someone) — the viewer's assignments in three tabs: My Tasks (awaiting outreach or a response), Wait, and All Done.
