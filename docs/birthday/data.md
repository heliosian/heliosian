...
`Default Charity` must name an allowed charity. `Email Subject` and `Email Body` are the outreach draft, with `{first name}`, `{name}`, `{birthday}`, `{newsletter date}`, `{default charity}`, `{sender}` (the signed-in person's name), and `{last year}` ("Last year you chose …", or nothing when there is no last year) filled in by the client; `No Newsletter Note` goes where `{no newsletter note}` sits in the body, or at the end when the body has no such place, for anyone whose participation is `No Newsletter`, and is blank for everyone else. Blank lines an empty placeholder leaves behind are dropped.
