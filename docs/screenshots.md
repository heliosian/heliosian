...
It takes `--width` and `--height` too, so `--width 390 --height 844` is a phone; add `?mode=dark` to the URL to see the page in dark mode. It marks Quan mode as already shown, so the sample super admin is not switched onto it and the capture wears the mode the URL asks for. `--click` and `--settle` are the same as `cmd/screenshot`'s below, and `--as <address>` views as that person through Spoof Mode, the way the sample super admin can from the toolbar, so a page can be captured as a plain member.
...
With the capture browser running, add `--remote` to attach to it instead of launching headless Chrome:
    ...
    go run ./cmd/screenshot --remote --url https://example.com/some/page --out local/screenshots/existing/page.png --wait body
...
`cmd/browse` drives the capture browser one step at a time: each invocation attaches to the current tab, performs at most one action, then captures and reports the resulting URL and title. The tab survives between invocations, so state (login, SPA position) carries across steps.
    ...
    go run ./cmd/browse --nav https://example.com/ --out local/screenshots/step1.png
    go run ./cmd/browse --clicksel "a.next" --wait "h1" --out local/screenshots/step2.png
    go run ./cmd/browse --click 640,300 --out local/screenshots/step3.png
...
The way to get a whole screen in one image is a tall `--size`:
    ...
    go run ./cmd/browse --size 1280x3400 --out local/screenshots/page.png
...
Past roughly 5000px a single image stops being readable. Capture those in parts at a fixed viewport, stepping with `--scroll` and watching the reported position for the point where it stops advancing:
    ...
    go run ./cmd/browse --size 1280x4000 --scroll -40000 --out local/screenshots/page-1.png
    go run ./cmd/browse --size 1280x4000 --scroll 4000 --out local/screenshots/page-2.png
