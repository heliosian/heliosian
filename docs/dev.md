...
- **Mail** — in sample mode the volunteer portal and Helios Celebrate write each email they would send as an `.html` file under `MAIL_DIR` (default `$TMPDIR/hca-mail`); open one in a browser to see it. Real-data mode sends through Resend when `creds/resend.key` (or `RESEND_KEY`) is present, else over SMTP when `SMTP_HOST` is set (with `SMTP_USER` and `creds/smtp.pass` or `SMTP_PASS`), else drops and logs. Replies to Helios Calendar's invites come back through a Resend webhook only in real-data mode with `creds/resend-webhook.secret` (or `RESEND_WEBHOOK_SECRET`) present; the sample server's invites name `rsvp@reply.example.org` and nothing receives for it.
