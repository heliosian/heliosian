...
- **Helios Loop's mail** — a real-data server forwards the posts to the groups through Resend with the same `creds/resend.key`, and takes them in only with `creds/loop-webhook.secret` (or `LOOP_WEBHOOK_SECRET`) present - which it is not on a laptop, since Resend's webhook points at production; without it the route answers 404. Every message it does forward is kept in the mail bucket as `directory@`, which the impersonated credentials above act as (`docs/groups/data.md`, Mail). Sample mode receives nothing, and a forward, were one to run, lands as `.eml` files under `MAIL_DIR` beside the other sample mail.
