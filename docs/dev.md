...
- `startserver` — the dev server: sample data in the foreground by default, `--capture` for a one-command page screenshot, `--real` for the production assembly in the foreground, and `--detach` to launch that in the background with its output in a log file plus a minted session cookie (needs `SESSION_KEY` and the six spreadsheet ids)
...
- `findsheet` — print the spreadsheet ids the server needs as shell exports, found by the sheets' titles (`Directory`, `Preferences`, `Invite List Builder`, `Apps`, `Events`, `Config`), with every other visible spreadsheet as a comment
