...
- `createtabs` — create the tabs of the `Directory`, `Apps`, and `Config` spreadsheets with their header rows, adding missing columns to tabs that exist (needs `DIRECTORY_SHEET`, `APPS_SHEET`, and `CONFIG_SHEET`, and refuses a spreadsheet whose title isn't the layout the variable promised)
