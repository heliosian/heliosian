...
- **Mail** — in sample mode the volunteer portal and Helios Celebrate write each email they would send as an `.html` file under `MAIL_DIR` (default `$TMPDIR/hca-mail`); open one in a browser to see it. Real-data mode sends through Mailgun when `creds/mailgun.key` (or `MAILGUN_KEY`) is present, else over SMTP when `SMTP_HOST` is set (with `SMTP_USER` and `creds/smtp.pass` or `SMTP_PASS`), else drops and logs. Replies to Helios Calendar's invites come back through a Mailgun route only in real-data mode with `creds/mailgun-webhook.key` (or `MAILGUN_WEBHOOK_KEY`) present too, and only if the route points at the server, which it does not on a laptop; the sample server's invites name `rsvp@reply.example.org` and nothing receives for it.
...
- **Feedback** — the toolbar's "Report a problem or idea" files issues in the private triage repository with the GitHub token in `creds/github.token` (or `GITHUB_TOKEN`), required in real-data mode; sample mode prints the issue it would file to the console instead (`docs/feedback.md`).
- **Helios Loop's mail** — a real-data server forwards the posts to the groups through Mailgun with `creds/mailgun.key` (or `MAILGUN_KEY`), and takes them in only with `creds/mailgun-webhook.key` (or `MAILGUN_WEBHOOK_KEY`) present too - and even then only if Mailgun's route points at it, which it does not on a laptop; without both the routes answer 404. Every message it does forward is kept in the mail bucket as `directory@`, which the impersonated credentials above act as (`docs/groups/data.md`, Mail). Sample mode receives nothing, and a forward, were one to run, lands as `.eml` files under `MAIL_DIR` beside the other sample mail.
...
- `startserver` — the dev server: sample data in the foreground by default, `--capture` for a one-command page screenshot, `--real` for the production assembly in the foreground with the media it fetches cached under `cache/blobs/` (Real data, above), and `--detach` to launch that in the background with its output in a log file plus a minted session cookie (needs `SESSION_KEY` and every spreadsheet id)
