...
- **Google Groups** — Helios Loop keeps real Google Groups in step through the Cloud Identity API as `directory@`, which the impersonated credentials above act as, so a real-data server on a laptop changes the production groups exactly as the deployed one does (`docs/groups/data.md`). Sample mode holds the groups in memory and logs every change instead.
...
- `periodicsync` — the scheduled job, run by hand: its stages in turn, each carrying on past the last's failure and the run exiting non-zero at the end naming what failed. The calendar stage pulls the school's public calendar feed and published year calendar into the `Calendar` sheet and has Claude classify every event (`docs/calendar/data.md`); the groups stage works out every group's members and brings the Google groups in step (`docs/groups/data.md`). `--dry-run` reports what either would change, writing nothing to the sheets or to Google; either way it refuses to start without `--i-have-user-permission-to-spend-money`, since every run that reaches Claude costs real money and only the person paying says when (needs `CALENDAR_SHEET`, `DIRECTORY_SHEET`, `PREFERENCES_SHEET`, `CONFIG_SHEET`, `EVENTS_SHEET`, `CELEBRATE_SHEET`, `GROUPS_SHEET`, an Anthropic API key in `creds/anthropic.key` or `ANTHROPIC_API_KEY` - the same key lets the dev server suggest charity sentences in Staff Birthdays; without it that button says it is not set up - and poppler's `pdftoppm`, `brew install poppler`)
