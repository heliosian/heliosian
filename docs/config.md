...
The `Signed Out` tab is written by sign-out itself: every app's `POST /auth/logout` records the moment for the address really signed in - the admin, under a spoof, never the person viewed as - through the same mirror-then-write path, and answers 500 with the browser's cookies left alone when the record refuses, so a session is never half ended. A super admin can end anyone's sessions with `POST /api/config/sign-out` and a body of `{"email": "..."}` on the directory's origin, which writes that address's row the same way; a regular admin gets the 403.
